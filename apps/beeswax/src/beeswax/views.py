    ...
    if request.method == 'POST':
        ...
        if None in designs.values():
            LOG.error('Cannot delete non-existent design(s) %s' % ','.join(
                [key for key, name in designs.items() if name is None]))
...
def restore_design(request):
    if request.method == 'POST':
        ...
        if None in designs.values():
            LOG.error('Cannot restore non-existent design(s) %s' % ','.join(
                [key for key, name in designs.items() if name is None]))
            return list_designs(request)
...
def clone_design(request, design_id):
    ...
    return format_preserving_redirect(request,
                                      reverse(get_app_name(request) + ':execute_design', kwargs={'design_id': copy.id}))
...
def massage_query_history_for_json(app_name, query_history):
    return {
        ...
        'resultsUrl': not query_history.is_failure() and reverse(app_name + ':watch_query_history',
                                                                 kwargs={'query_history_id': query_history.id}) or ""
    }
...
def item_query(request, design_id=None, query_history_id=None):
    """
    View function for executing an arbitrary query.
    """
    action = 'item query'
    ...
    return render('item_query.mako', request, context)
...
def _list_designs(user, querydict, page_size, prefix="", is_trashed=False):
    ...
    designs = [job.content_object for job in db_queryset.all() if
               job.content_object and job.content_object.is_auto == False]
...
def _update_query_state(query_history):
    ...
    if query_history.last_state <= models.QueryHistory.STATE.running.index:
        try:
            state_enum = dbms.get(query_history.owner, query_history.get_query_server_config()).get_state(
